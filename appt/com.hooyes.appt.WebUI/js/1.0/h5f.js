...
function ($) {
    ...
    function scalePage() {
        ...
        320 != clientWidth && clientWidth == document.documentElement.clientWidth || isWeixin() 
...
$(function(){
    ...
    for(var i=0;i<h5data.length;i++){
    	var spage= h5data[i];
	    var r_html=$("#"+spage.tid).html();
	    for(var s in spage){
	      var reg="@@"+s;
          r_html=r_html.replace(new RegExp(reg,'ig'),eval("spage."+s))
	      r_html=r_html.replace(/@@FILE_CDN/ig,FILE_CDN);
		}

		$("#main_page_container").append(r_html);
	}
    var _spage=$.Request('p');
    if(_spage>0 && _spage<h5data.length){

    }else{
        _spage=0;
    }
    $("#main_page_container").find(".main-page:eq("+_spage+")").addClass("z-current")
