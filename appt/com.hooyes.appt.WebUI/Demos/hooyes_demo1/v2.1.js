...
function nextPage() {
    ...
    var b = setInterval(function () {
        ...
        moveDistanceY = a,
        scrollMove(),
        -21 >= a && (clearInterval(b), scrollEnd())
    },
    1)
} !
function (a) {
...
function ($) {
    ...
    function addElmentsForPc(a) {
        ...
        c.href = CLIENT_CDN + "pcviewer.css",
        c.rel = "stylesheet";
    ...
    function parseJson(a) {
        ...
        d = !1,
        e = a.obj.image;
        if (b.length <= 0) return alert("此场景不存在"),
        void (window.location.href = "http://hooyes.com");
        customLastPage = !1,
        appendLastPage(a, b),
        bindShare(a),
        document.title = a.obj.name,
        $("#metaDescription").attr("content", a.obj.name + ", " + a.obj.description + ", hooyes!"),
        $(".scene_title").html(a.obj.name);
        for (var f = 1; f <= b.length; f++)
            if ($('<section class="main-page app-edit" ><div class="m-img" id="page' + f + '"></div></section>').appendTo(".nr"), b.length > 1 && $('<section class="u-arrow"><img src="' + CLIENT_CDN + 'view/images/btn01_arrow.png" /></section>').appendTo("#page" + f), 1 == f && ($(".loading").hide(), $(".main-page").eq(0).addClass("z-current")), b[f - 1].properties ? b[f - 1].properties.image || b[f - 1].properties.scratch ? scriptLoaded.scratch ? addScratchEffect(b, f) : !
        function (a) {
            $.getScript(CLIENT_CDN + "view/js/scratch_effect.js",
            function () {
                scriptLoaded.scratch = !0,
                addScratchEffect(e, b, a)
            })
            }(f) : b[f - 1].properties.finger ? (c.push({
                num: f,
                finger: b[f - 1].properties.finger
            }), d || (d = !0, $.getScript(CLIENT_CDN + "view/js/lock_effect.js",
        function () {
            test(e, b, c, $(".m-img").width(), $(".m-img").height())
                ...
                //preview || $.ajax({
                //    type: "GET",
                //    url: PREFIX_URL + "eqs/pv/" + a.obj.id,
                //    xhrFields: {
                //        withCredentials: !0
                //    },
                //    crossDomain: !0
    ...
    function scalePage() {
        ...
        //    marginLeft: a
        //}),
        //    //$("#eqMobileViewport").attr("content", "width=320, initial-scale=" + c + ", maximum-scale=" + c + ", user-scalable=no"),
